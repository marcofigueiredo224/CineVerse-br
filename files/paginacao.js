...
// Paginas
let perPage = 12; // Alterado para 12
const state = {
  page: 1,
  perPage,
...
const controls = {
  ...
  goTo(page) {
    // se for passado uma pagina menor que 1, ele vai cair nesse if.
    if (page < 1) {
      page = 1;
    }
...
const list = {
  ...
  update() {
    ...
    const images = document.querySelectorAll('.img-series'); // Seleciona todas as imagens com a classe 'img-series'

    for (let i = start; i < end; i++) {
      if (images[i]) {
        html.get('.list').appendChild(images[i].cloneNode(true)); // Adiciona as imagens à lista
      }
    }
  }
};
...
const buttons = {
  ...
  update() {
    // zerando tudo que tem lá dentro
